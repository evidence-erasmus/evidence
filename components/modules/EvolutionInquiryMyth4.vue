<script setup>
    ...
    const studentAnswers = useStudentEvolutionGuidedMyth4();
...
<template>
    ...
    <div class="flex flex-col items-start 
        ...
        <section class="e-article print:not-prose" :lang="locale" style="word-wrap: break-word; overflow-wrap: break-word; -webkit-hyphens: auto; -moz-hyphens: auto; hyphens: auto;">
            ...
            <h1 class="mb-0">{{ t('inquiry.ev_myth4.title') }}</h1>
            <h5 class="font-normal uppercase leading-tight">{{ t('inquiry.ev_myth4.subtitle') }}</h5>
        

            <UiCardInfo theme="rose-400" icon="game-icons:unicorn" class="my-16">
                <h4 class="leading-tight my-2">
                    {{ t('inquiry.ev_myth4.myth.text') }} {{ t('inquiry.ev_myth4.myth.myth') }}
                </h4>
            </UiCardInfo>

            
            <p>
                {{ t('inquiry.ev_myth4.justification.text') }}
            </p>

            <label for="q1" class="block mb-2">
                <b>{{ t('inquiry.ev_myth4.justification.label_1') }}</b>
            </label>
            ...
            <label for="q2" class="block mb-2">
                <b>{{ t('inquiry.ev_myth4.label_2') }}</b>
            </label>
            ...
            <label for="q3" class="block mb-2">
                <b>{{ t('inquiry.ev_myth4.label_3') }}</b>
            </label>
            ...
            <UiCardInfo icon="mdi:question-mark-circle" theme="sky-500">
                <h4 class="my-4 leading-tight">
                    {{ t('inquiry.ev_myth4.question') }}
                </h4>
            </UiCardInfo>
            

            <p>
                {{ t('inquiry.ev_myth4.sources_text') }}
            </p>
            <ol class="leading-tight mb-16">
                <li v-for="li in tm('inquiry.ev_myth4.sources')">
            ...
            <i18n-t tag="p" keypath="inquiry.ev_myth4.reliability.text" scope="global">
                <template #link>
                    <NuxtLink to="../../tools/reliability-assessment" target="_blank" class="btn btn-sm btn-neutral text-white no-underline"><Icon name="mdi:tools" size="18" />
                        {{ t('inquiry.ev_myth4.reliability.link') }}
                    </NuxtLink>
                </template>
            </i18n-t>
            
            <label class="block mb-2">
                <b>{{ t('inquiry.ev_myth4.reliability.label') }}</b>
            </label>
            <div class="e-card">
                <ClientOnly>
                    <ToolsReliabilityTable :storeToUpdate="studentAnswers"
                    :addRow="true" />
                </ClientOnly>
            </div>
